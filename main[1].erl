...
server(N,Topp,Algorithm)->
    receive
        hello ->
            OldWorkers= birth_workers(N, Algorithm),
            io:format("Worker Process ids in the list are ~p~n",[A]),
            case Topp of
        ...
        heard -> 
            if
                HeardNodes+1==TotalNodes ->
                    io:format("Gossip Updated and Converged~n"),
                    spid ! done;
                true ->
                    tracker(TotalNodes,HeardNodes+1,ReadyNodes)
            end;
        {heard, Ratio} ->
            io:format("Push Sum has Converged and the ratio is ~p~n",[Ratio]),
            spid ! done;
        iamready ->
            if
...
start(N,Topp, Algorithm)->
    io:format("Total number of workers: ~p~n",[N]),
    ...
    io:format("Final Number of workers after rounding up are: ~p~n",[FinalNodeCount]),
